package com.levin.framework.springmvc.framework.servlet;
...
import com.levin.framework.springmvc.framework.context.ApplicationContext;
...
public class DispatcherServlet extends HttpServlet {
    ...
    private Map<Handler, HandlerApapter> adapaterMapping = new HashMap<>();
    ...
    private void initViewResolvers(ApplicationContext context) {
        ...
        //归根到底就是一个文件，普通文件
        String rootPath = Objects.requireNonNull(this.getClass().getClassLoader().getResource(templateRoot)).getFile();

        File rootDir = new File(rootPath);
        for (File template : Objects.requireNonNull(rootDir.listFiles())) {
    ...
    public void applyDefaultViewName(HttpServletResponse response, ModelAndView mv) throws Exception {
        if (null == mv) {
            return;
        }
    ...
    private class HandlerApapter {
         ...
         */
        public ModelAndView handle(HttpServletRequest request, HttpServletResponse response, Handler handler) throws InvocationTargetException, IllegalAccessException {
